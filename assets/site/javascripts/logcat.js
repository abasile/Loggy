...
var colors = [ '#FFFFFF', '#FF0000', '#00FF00', '#0000FF', '#0FF000', '#000FF0', '#F0000F', '#F0F000', '#F00F00', '#F000F0', '#0F0F00', '#00F00F', '#0F00F0' ];
var tags = {};
...
ws.onmessage = function(e) {
	...
	var format =
		'.row.hide.log.logpid-' + pid + '.logtag-' + tag + '\n' +
		'  .span1.logpid=pid\n' +
		'  .span2.logtag(style="color: ' + color + ';")=tag\n' +
		'  .span9\n' +
		'    span.logtype.' + type + '=type\n' +
		'    span=text\n';
	jade.render(format, {
		type: type,
		tag: tag,
		...
		data = $(data).click(applyFilter);
		console.log(data);
		if ($(data).hasClass(currentLogFilter))
			$(data).show();
		$('#log-rows').append(data);
		if (t) {
			clearTimeout(t);
			t = null;
		}
...
function clearFilters() {
	...
	$('#current-filter').hide();
	$("html, body").animate({ scrollTop: $(document).height() }, 200);
	currentLogFilter = 'log';
}

var currentLogFilter = 'log';
function applyFilter(event) {
	if ($(event.target).hasClass('logpid')) {
		var pid = $(event.target).html();
		$('.log').hide();
		currentLogFilter = 'logpid-' + pid;
		$('.' + currentLogFilter).show();
		$('#no-filter').hide();
		$('#clear-filter').show();
		$('#current-filter').show();
		$('#current-filter-value').html(pid);
	}
	else if ($(event.target).hasClass('logtag')) {
		var tag = $(event.target).html();
		$('.log').hide();
		currentLogFilter = 'logtag-' + tag;
		$('.' + currentLogFilter).show();
		$('#no-filter').hide();
		$('#clear-filter').show();
		$('#current-filter').show();
		$('#current-filter-value').html(tag);
	}
}
